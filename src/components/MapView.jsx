...
import {
  noiseData, pedestrianPolygons, parkPolygons,
  WEIMAR, ROUTE_CONFIG, CAR_COLORS,
} from '../data/mapData'

// ── SVG hatch patterns injected into Leaflet's SVG overlay ───────────────────
function SvgPatterns() {
  const map = useMap()
  useLayoutEffect(() => {
    // Force Leaflet to create its SVG overlay by briefly adding a dummy layer
      ...
      defs.innerHTML = `
        <pattern id="ped-hatch" width="8" height="8"
          ...
          <line x1="0" y1="0" x2="0" y2="8"
                stroke="#fde047" stroke-width="2.5" stroke-opacity="0.55"/>
        </pattern>
        <pattern id="park-hatch" width="9" height="9"
            patternUnits="userSpaceOnUse" patternTransform="rotate(135)">
          <rect width="9" height="9" fill="rgba(74,222,128,0.08)"/>
          <line x1="0" y1="0" x2="0" y2="9"
                stroke="#4ade80" stroke-width="2.5" stroke-opacity="0.55"/>
        </pattern>
      `
...
function MapInteraction({ onMapClick, isPlacing }) {
  ...
  useEffect(() => {
    ...
    if (isPlacing) {
      el.classList.add('cursor-flag')
    } else {
      el.classList.remove('cursor-flag')
    }
...
// ── Noise colour (db → colour, clearer scale) ─────────────────────────────────
function noiseColor(db) {
  if (db >= 75) return '#7f1d1d' // very loud — dark red
  if (db >= 65) return '#dc2626' // loud — red
  if (db >= 55) return '#f97316' // moderate — orange
  if (db >= 45) return '#eab308' // low — yellow
...
function makeCarIcon(bearing, color) {
  return L.divIcon({
    html: `<div style="transform:rotate(${bearing}deg);width:18px;height:26px;
                       filter:drop-shadow(0 2px 4px rgba(0,0,0,0.6))">
      <svg viewBox="0 0 18 26" xmlns="http://www.w3.org/2000/svg">
        <rect x="1" y="3" width="16" height="20" rx="3"
              fill="${color}" stroke="white" stroke-width="0.8"/>
        <rect x="3" y="5" width="12" height="6" rx="1"
        ...
        return (
          <LayerGroup key={i}>
            <Marker position={p.ll} icon={L.divIcon({
              html: `<div style="font-size:18px;line-height:1;
                         filter:drop-shadow(0 1px 3px #000)">${p.icon}</div>`,
              className: '', iconAnchor: [9, 9],
            })}>
              <Popup>
                <b>{p.icon} {p.label}</b><br />
                <span style={{ color: col }}>{p.db} dB(A) estimated</span>
              </Popup>
            </Marker>
            {/* Outer ring: very transparent fill, clear coloured border */}
            <Circle center={p.ll} radius={p.db * 3}
              pathOptions={{ color: col, fillColor: col, fillOpacity: 0.04, weight: 2, opacity: 0.45, dashArray: '4 4' }} />
            {/* Inner ring: slightly more visible */}
...
export default function MapView({
  ...
  // Show nothing until OSRM has loaded real geometries
  const activeRoads = fetchedRoads ?? []
  const activeBikeLanes = fetchedBikeLanes ?? []
  ...
  return (
    <MapContainer center={WEIMAR} zoom={14} zoomControl={false} className="h-full w-full">
      ...
      {/* Must be before any Polygon that uses patterns */}
      <SvgPatterns />
      ...
      {/* Route endpoints */}
      {ptA && <Marker position={[ptA.lat, ptA.lng]} icon={iconA}><Popup>Start point A</Popup></Marker>}
      ...
      {/* Noise source layers */}
      {['traffic-noise','rail-noise','construction','school','hospitality','park'].map(key =>
      ...
      {/* Pedestrian zones — light yellow hatch polygon */}
      {layerVisibility['pedestrian'] && (
        <LayerGroup>
          {pedestrianPolygons.map((poly, i) => (
            <Polygon key={i} positions={poly}
              pathOptions={{
                fillColor: 'url(#ped-hatch)',
                fillOpacity: 1,
                color: '#fde047',
                weight: 1.5,
                opacity: 0.6,
      ...
      {/* Park / green zones — green hatch polygon */}
      {layerVisibility['park'] && (
        <LayerGroup>
          {parkPolygons.map((poly, i) => (
            <Polygon key={i} positions={poly}
              pathOptions={{
                fillColor: 'url(#park-hatch)',
                fillOpacity: 1,
                color: '#4ade80',
                weight: 1.5,
                opacity: 0.6,
              }}>
              <Popup><b>🌳 Green zone / park</b></Popup>
            </Polygon>
          ))}
        </LayerGroup>
      )}
      ...
      {liveOn && fetchedCarPaths && (
        <LayerGroup>
          {fetchedCarPaths.map((path, i) => {
            ...
            return (
              <Marker key={i} position={path[pos]} icon={makeCarIcon(bearing, CAR_COLORS[i])} />
            )
          })}
        </LayerGroup>
      )}
